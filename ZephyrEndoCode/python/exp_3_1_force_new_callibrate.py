                ...
                if (i % (1000/PRNWAIT)) == 0:
                    if pattern_count % len(actuator_pattern) == 0:
                        ...
                        if pattern_count // len(actuator_pattern) >= n_cycle:
                            break
                    regulator_vals[3] = backbone_pressure
                    regulator_vals[5] = actuator_pattern[pattern_count % len(actuator_pattern)]  # channel 6
                    pattern_count += 1
                    makePressureCmd(regulator_vals)
                    print('n', pattern_count, 't', time.time() - t0, 'regulator', regulator_vals)
