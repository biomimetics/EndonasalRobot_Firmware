...
def control_loop(): 
    ...
    pressure_value5 = setPressure(regulator5)
    pressure_value6 = setPressure(regulator6)
    pressure_value7 = setPressure(regulator7)
    pressure_value8 = setPressure(regulator8)
    # message = makeCmdString('PWM1', pressure_value1) + makeCmdString('PWM2', pressure_value2) + makeCmdString('PWM3', pressure_value3) + makeCmdString('PWM4', pressure_value4) + makeCmdString('PWM5', pressure_value5)+ makeCmdString('PWM6', pressure_value6)+ makeCmdString('PWM7', pressure_value7)+ makeCmdString('PWM8', pressure_value8)
    # sendQ.put(message)
    # message = makeCmdString('OUTON', 4)
    # sendQ.put(message)
    # while ((i < 10000) and (not controlStop.is_set())):
    while (not controlStop.is_set()):
        if not stateQ.empty():
            ...
            pressure_value1 = setPressure(regulator1)
            pressure_value2 = setPressure(regulator2)
            # pressure_value1 = setPressure(10*np.cos(0.1*np.pi*(time.time()-t0))+15)
            # pressure_value2 = setPressure(10*np.sin(0.1*np.pi*(time.time()-t0))+15)
            pressure_value3 = setPressure(regulator3)
            pressure_value4 = setPressure(regulator4)
            pressure_value5 = setPressure(regulator5)
            pressure_value6 = setPressure(regulator6)
            ...
            sendQ.put(message+b'\n')
            message = makeCmdString('PWM5', pressure_value5)+ makeCmdString('PWM6', pressure_value6) +  makeCmdString('PWM7', pressure_value7)+ makeCmdString('PWM8', pressure_value8)
            sendQ.put(message+b'\n')
            if solenoid1 == 0:
                message = makeCmdString('OUTOFF', 1)
            else: message = makeCmdString('OUTON', 1)
            if solenoid2 == 0:
                message = message + makeCmdString('OUTOFF', 2)
            else:  message = message +  makeCmdString('OUTON', 2)
            if solenoid3 == 0:
                message = message + makeCmdString('OUTOFF', 3)
            else:  message = message +  makeCmdString('OUTON', 3)
            if solenoid4 == 0:
                message = message + makeCmdString('OUTOFF', 4)
            else:  message = message +  makeCmdString('OUTON', 4)
            sendQ.put(message+b'\n')
