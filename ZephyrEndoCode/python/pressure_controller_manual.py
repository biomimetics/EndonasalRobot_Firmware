...
def processLine(textLine,index):
 ...
 #           print(textLine)
  ...
  #          print(index, end=',')
        ...
        # sensor3 = convertGagePressureSmall(convertADC(newState.adc11))
        # print(sensor3)
        # sensor4 = convertGagePressureSmall(convertADC(newState.adc10))
        #print(sensor4)
        if sensor1 != None:
...
def input_thread():
    global regulator1, regulator2, regulator3, regulator4
    while True:
        try:
            input_values = input("Enter values for regulator 1 and 2 (separated by a space): ")
            new_regulator1, new_regulator2, new_regulator3, new_regulator4 = [float(val) for val in input_values.split()]
